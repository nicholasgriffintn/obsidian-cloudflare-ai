...
export class TextGenerationService {
    ...
    async generateFromTemplate(template: Template, variables: Record<string, string>): Promise<string> {
        const prompt = await this.templateManager.applyTemplate(template, variables);
    ...
    async generateInEditor(editor: Editor, options: {
        templateName?: string,
        ...
        try {
            ...
            new Notice("Failed to generate text: " + (error instanceof Error ? error.message : String(error)));
        }
    }
