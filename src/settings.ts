                        ...
                        if (temporaryKey) {
                            try {
                                if (safeStorage.isEncryptionAvailable()) {
                                    ...
                                    this.plugin.settings[keyField] = Buffer.from(encrypted).toString('base64');
                                } else {
                ...
                dropdown
                    .addOptions(Object.fromEntries(
                        Object.entries(ModelIds).map(([_, value]) => [
                            value,
                            value.split('/').pop()?.replace(/-/g, ' ').toUpperCase() ?? value
                        ])
                    ))
                ...
                text
                    ...
                    .onChange(async (value) => {
                        ...
                        if (!isNaN(parsedValue) && parsedValue >= min && parsedValue <= max) {
                            ...
                            new Notice(`Invalid value for ${name}. Must be between ${min} and ${max}`);
                        }
                    }),
            );
    }
